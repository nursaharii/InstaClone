...
class FeedVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getDataFromFireStore(){
        ...
        if error != nil{
            ...
            if snapshot?.isEmpty != nil {
                self.img.removeAll(keepingCapacity: false)
                self.user.removeAll(keepingCapacity: false)
                self.comment.removeAll(keepingCapacity: false)
                self.counter.removeAll(keepingCapacity: false)
                self.docArry.removeAll(keepingCapacity: false)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.img.sd_setImage(with: URL(string: self.img[indexPath.row]))
